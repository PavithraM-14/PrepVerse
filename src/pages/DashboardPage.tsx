      ...
      <div className="p-4 md:p-6 space-y-6 max-w-7xl mx-auto">
          ...
          {[
            {
              ...
              sub: 'days', color: 'text-warning', bgGrad: 'from-orange-500/10 to-yellow-500/5'
            },
            {
              icon: Trophy, label: 'XP Points', value: loading ? '-' : (progress?.xp_points ?? 0).toLocaleString(),
              sub: `Level ${level}`, color: 'text-neon-purple', bgGrad: 'from-purple-500/10 to-blue-500/5'
            },
            {
              icon: Target, label: 'Resume Score', value: loading ? '-' : `${progress?.resume_score ?? 0}%`,
              sub: 'ATS optimized', color: 'text-neon-blue', bgGrad: 'from-blue-500/10 to-cyan-500/5'
            },
            {
              icon: Zap, label: 'Interview Ready', value: loading ? '-' : `${progress?.interview_readiness ?? 0}%`,
              sub: 'readiness', color: 'text-success', bgGrad: 'from-green-500/10 to-emerald-500/5'
            },
          ].map((s) => (
            <Card key={s.label} className={`card-hover bg-gradient-to-br ${s.bgGrad} border-border/60`}>
        ...
        <Card className="border-border/60">
          <CardContent className="p-4">
            ...
            <Progress value={xpProgress} className="h-3 [&>div]:xp-bar" />
            ...
            <CardContent className="flex-1 grid grid-cols-2 gap-3 content-start">
              {quickActions.map((a) => (
                <button
                  ...
                  className={`${a.bg} rounded-xl p-4 text-left hover:opacity-80 transition-opacity card-hover flex flex-col gap-2`}
                >
                  <a.icon className={`w-5 h-5 ${a.color}`} />
                  <span className="text-sm font-medium text-balance">{a.label}</span>
                </button>
              ))}
            </CardContent>
          </Card>
          ...
          <Card className="border-border/60 h-full flex flex-col">
            <CardHeader className="pb-3">
              <div className="flex items-center justify-between">
                <CardTitle className="text-base flex items-center gap-2">
                  <CheckCircle className="w-4 h-4 text-success" /> Daily Goals
                </CardTitle>
            ...
            <CardContent className="flex-1 overflow-y-auto min-h-0">
              {loading ? (
                ...
                <div className="text-center py-6 text-muted-foreground text-sm">
                  ...
                  <p>No tasks yet.</p>
                  <Button size="sm" variant="outline" className="mt-3" onClick={() => navigate('/planner')}>
                    Create Tasks
                  </Button>
                </div>
              ) : (
                <div className="space-y-2">
                  {tasks.map((task) => (
                    ...
                    >
                      ...
                      <span className={`text-sm flex-1 min-w-0 truncate ${task.is_completed ? 'line-through text-muted-foreground' : ''}`}>
                        {task.title}
                      </span>
                      <Badge variant="secondary" className="text-xs shrink-0">+{task.xp_reward} XP</Badge>
                    </div>
                  ))}
                </div>
              )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <Card className="border-border/60 bg-gradient-to-br from-primary/5 to-neon-purple/5">
            <CardHeader className="pb-3">
              <div className="flex items-center justify-between">
                <CardTitle className="text-base flex items-center gap-2">
                  <Brain className="w-4 h-4 text-primary" /> AI Tip of the Day
                </CardTitle>
                <Button size="sm" variant="ghost" onClick={generateAiTip} disabled={tipLoading}>
            ...
            <CardContent>
              {aiTip ? (
                <Streamdown parseIncompleteMarkdown isAnimating={tipLoading} className="text-sm text-muted-foreground [&>*]:m-0">
                  {aiTip}
                </Streamdown>
              ) : (
                <p className="text-sm text-muted-foreground text-pretty">{getTipOfDay(progress)}</p>
              )}
              {!aiTip && (
                <Button size="sm" variant="outline" className="mt-3" onClick={generateAiTip} disabled={tipLoading}>
          ...
          <Card className="border-border/60 h-full flex flex-col">
            <CardHeader className="pb-3">
              <div className="flex items-center justify-between">
                ...
                {notifications.length > 0 && (
                  <Badge className="bg-destructive/10 text-destructive border-0 text-xs">{notifications.length}</Badge>
                )}
              </div>
            </CardHeader>
            <CardContent className="flex-1">
              {notifications.length === 0 ? (
                ...
                <div className="space-y-2">
                  {notifications.map((n) => (
                    <div key={n.id} className="flex items-start gap-3 p-2 rounded-xl bg-accent/30">
        ...
        {/* Community teaser */}
        <Card className="border-border/60 bg-gradient-to-br from-neon-purple/5 to-primary/5">
          <CardContent className="p-4 flex items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              <Users className="w-8 h-8 text-neon-purple shrink-0" />
              <div>
                <p className="font-semibold text-sm">Join the Community Challenge</p>
                <p className="text-xs text-muted-foreground">Compete with 200+ students this week</p>
              </div>
            </div>
